...
sdk
  ...
  .then(() => {
    console.log(
      "OpenTelemetry tracing initialized successfully for CCXT service",
    );
    console.log(`Service: ${serviceName} v${serviceVersion}`);
    if (otlpTracesEnv) {
      console.log(
        `OTLP Endpoint (from OTEL_EXPORTER_OTLP_TRACES_ENDPOINT): ${resolvedOtlpEndpoint}`,
      );
    } else if (otlpBaseEnv) {
      console.log(
        `OTLP Endpoint (normalized from OTEL_EXPORTER_OTLP_ENDPOINT): ${resolvedOtlpEndpoint}`,
      );
    } else {
