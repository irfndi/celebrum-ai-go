...
async function runTests() {
  ...
  try {
    ...
    console.log("\n--- Test 2: Exchanges endpoint ---");
    const exchangesRes = await service.fetch(
    ...
    if (
      exchangesBody &&
      exchangesBody.exchanges &&
      Array.isArray(exchangesBody.exchanges)
    ) {
      ...
      if (count > 0) {
        ...
        if (typeof firstElement === "string") {
          ...
          hasBinance = exchangesBody.exchanges.some(
            (item) =>
    ...
    console.log("\n--- Test 3: Markets endpoint ---");
    const marketsRes = await service.fetch(
    ...
    console.log("\n--- Test 4: Ticker endpoint (valid) ---");
    const tickerRes = await service.fetch(
    ...
    console.log("\n--- Test 5: Ticker endpoint (invalid exchange) ---");
    const invalidTickerRes = await service.fetch(
    ...
    console.log("\n--- Test 6: Funding rates endpoint ---");
    const fundingRes = await service.fetch(
    ...
    console.log(
      ...
      fundingBody.count || "error",
      fundingBody.error || "success",
    );
