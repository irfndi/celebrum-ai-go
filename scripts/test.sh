...
build_test_services() {
    ...
    case "$test_type" in
        "ci")
            if $docker_compose_cmd -f "${COMPOSE_FILE:-docker-compose.yml}" build --no-cache; then
        ...
        "core")
            ...
            local core_services=("postgres" "app")
...
start_test_services() {
    ...
    case "$test_type" in
        "ci")
            if $docker_compose_cmd -f "${COMPOSE_FILE:-docker-compose.yml}" up -d; then
...
wait_for_services() {
    ...
    case "$test_type" in
        "ci")
            services_to_check=($($docker_compose_cmd -f "${COMPOSE_FILE:-docker-compose.yml}" config --services))
            ;;
        "core")
            services_to_check=("postgres" "app")
            ;;
        "full")
...
test_ccxt_service() {
    ...
    # Check if Celebrum (unified) service is running
    if $docker_compose_cmd ps celebrum | grep -q "Up"; then
        log_success "Celebrum unified service is running (hosting CCXT)"
        ...
        log_warn "Celebrum service is not running"
    fi
...
Test Types:
  ...
  core       - Core services only (postgres, app)
