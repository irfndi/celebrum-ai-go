...
const createMockApi = (): Api => ({
    getUserByChatId: mock(() => Effect.succeed({ user: { id: "user123", subscription_tier: "Premium", created_at: "2023-01-01" } })),
    getNotificationPreference: mock(() => Effect.succeed({ enabled: true, profit_threshold: 0.5, alert_frequency: "Every 5 minutes" })),
    setNotificationPreference: mock(() => Effect.succeed({ status: "success", enabled: true })),
    registerTelegramUser: mock(() => Effect.succeed({})),
    getOpportunities: mock(() => Effect.succeed({ opportunities: [{ symbol: "BTC/USDT", buy_exchange: "ex1", buy_price: 100, sell_exchange: "ex2", sell_price: 110, profit_percent: 10 }] })),
} as any);
...
describe("Bot Handlers", () => {
    ...
    test("handleStart registers user if not found", async () => {
        ...
        (api.getUserByChatId as any).mockImplementationOnce(() => Effect.fail(new Error("Not found")));
    ...
    test("handleOpportunities handles error", async () => {
        const api = createMockApi();
        (api.getOpportunities as any).mockImplementation(() => Effect.fail(new Error("Network Error")));
        const ctx = createMockContext();

        await handleOpportunities(api)(ctx);

        expect(ctx.reply).toHaveBeenCalled();
        expect(ctx.reply.mock.calls[0][0]).toContain("Failed to fetch opportunities");
    });
    ...
    test("handleStatus asks to register if user not found", async () => {
        const api = createMockApi();
        (api.getUserByChatId as any).mockImplementation(() => Effect.fail(new Error("Not found")));
        const ctx = createMockContext();
        await handleStatus(api)(ctx);
        expect(ctx.reply).toHaveBeenCalled();
    ...
    test("handleStop disables notifications", async () => {
        ...
        expect(api.setNotificationPreference).toHaveBeenCalledWith(expect.any(String), false);
        expect(ctx.reply).toHaveBeenCalled();
    ...
    test("handleResume enables notifications", async () => {
        ...
        expect(api.setNotificationPreference).toHaveBeenCalledWith(expect.any(String), true);
        expect(ctx.reply).toHaveBeenCalled();
