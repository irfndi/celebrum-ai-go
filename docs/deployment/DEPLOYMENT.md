...
- Keep Coolify env vars aligned with runtime: DB/Redis hosts/passwords, `CCXT_SERVICE_URL=http://127.0.0.1:3000`, `PORT=3000`, `BUN_NO_SERVER=false` (or unset), and the numeric DB tuning knobs (`DATABASE_MAX_OPEN_CONNS`, `DATABASE_MAX_IDLE_CONNS`, timeouts, etc.).
