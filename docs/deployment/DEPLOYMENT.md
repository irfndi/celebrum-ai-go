...
- **Bun double-serve (EADDRINUSE)**: If CCXT logs `Failed to start server. Is port 3000 in use?`, ensure only one `Bun.serve` runs. The code now supports `BUN_NO_SERVER=true` or `CCXT_AUTO_SERVE=false` to skip auto-serve; otherwise it will serve once on `PORT`. Avoid extra entrypoints that also call `Bun.serve`.
...
- Keep Coolify env vars aligned with runtime: DB/Redis hosts/passwords, `CCXT_SERVICE_URL=http://127.0.0.1:3000`, `PORT=3000`, `BUN_NO_SERVER=false` (or unset), and the numeric DB tuning knobs (`DATABASE_MAX_OPEN_CONNS`, `DATABASE_MAX_IDLE_CONNS`, timeouts, etc.) as numbers only.
