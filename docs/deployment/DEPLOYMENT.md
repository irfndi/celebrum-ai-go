...
- **Bun double-serve (EADDRINUSE)**: If CCXT logs `Failed to start server. Is port 3000 in use?`, ensure only one `Bun.serve` runs. With the dist override, add a single manual `Bun.serve({ fetch: app.fetch, port: PORT || 3000 })` and set `BUN_NO_SERVER=false` so Bun doesn’t start twice.
